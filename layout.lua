...
function LayoutManager:init()
    ...
    screen.width = app:getContentWidth()
    screen.height = app:getContentHeight()
    ...
    -- NOTE: Corona to Gideros
    -- TODO: Find topStatusBarContentHeight for Gideros
    ...
    stage.height = screen.height - statusBarHeight
    ...
    -- NOTE: Corona to Gideros
    local display = {
        pixelWidth = app:getDeviceWidth(),
        pixelHeight = app:getDeviceHeight()
    }
...
function LayoutManager:addRegion(dimens)
...
function LayoutManager:removeRegion(id)
...
function LayoutManager:adjustRegion(dimens)
    assert(dimens.id, "Region id not defined")
    assert(self[dimens.id], "Region does not exist")
    assert(
        self[dimens.id].user,
        "Can't adjust non-user region"
    )
    local region = self[dimens.id]
